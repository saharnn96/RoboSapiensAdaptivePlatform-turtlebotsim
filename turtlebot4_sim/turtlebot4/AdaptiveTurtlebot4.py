...
from turtlebot4_sim.turtlebot4.Camera import Camera
...
from RoboSapiensAdaptivePlatform.ManagedSystem.ManagedSystemTemplates import remoteManagedSystem
from RoboSapiensAdaptivePlatform.Communication.Messages.messages import *


class adaptiveTurtlebot4(remoteManagedSystem):

    def __init__(self,x_start=0.0,y_start=0.0,theta_start=0.0,showAnimation = True,config='00_input/config.yaml',verbose=False):
        super().__init__(name="adaptiveTurtlebot4", description="Adaptive version of the turtlebot 4 - MQTT interface with RaP", logger=None, effector=None, probe=None,config=config,verbose=verbose)
        ...
        # virtual persons running around in the environment
        self._personList = []

        # virtual camera
        self._camera = Camera(position=[self._x, self._y],Range=30.0,FOV=120.0,angularResolution=0.5,uncertentity=(0.5, 0.01),personList=self._personList)
        self._camera_executor = perpetualTimer(1, self._camera_callback)  # execute every 1 seconds
        self._camera_executor.start()

        # --- Overload the effector spin function if RaP is passed ---
        self._RaPEffector._SpinOnceFcn = self._effector_spin_once
    ...
    def navigate_waypoints(self,target_speed=3.5):

        #self.target_speed = target_speed   # target speed set upfront and adapted at runtime
    ...
    def _camera_callback(self):
        self._camera.spin_once(self._x, self._y,self._theta)
    ...
    def _plot(self):
        ...
        plot_arrow(self._x, self._y, self._theta,length=2.0,width=5.0,fc='b',ec='b')

        # --- plot turtlebot trajectory ---
        ...
        # --- plot all persons as arrow ---
        for person in self._personList:
            plot_arrow(person._x, person._y, person._theta,length=5.0,width=10.0,fc='g',ec='g')
    ...
    def _probes_update(self):
        # --- probe robot pose ---
        robotPose = RobotPose(name="RobotOdometry", frameID="FID_001", confidence=99,
                              position=[self._x, self._y, 0.0],
                              orientation=[self._x, self._y, 0.0, self._theta],
                              linear=[0.0, 0.0, 0.0, 0.0],
                              angular=[0.0, 0.0, 0.0, 0.0])  # TODO: add twist info (linear,angular)
        self.RaPProbe.push('RobotOdometry', robotPose)

        # --- probe person detection ---
        detectedObjects = self._camera._objectList
    ...
    def _effector_spin_once(self,args):
        ...
        # 1. FETCH ALL DECODED ACTIONS IF NEEDED
        actionList = self.RaPEffector.getActions(endpointName="GenericAction")
        action = actionList[-1] #TODO: CHECK IF ONLY LAST ACTION NEEDS TO BE PERFORMED

        # 2. DECODE THE ACTION TO BE PERFORMED
        _rawAction = action
        ...
        if _actionType == actionType.DIAGNOSISTYPE:
            # 3a. PERFORM THE DIAGNOSE ACTION ROUTINE
            #self.RaPLogger.log("["+self._name+"] - "+'Managed system diagnose - "' + _description + '"')
            if self._verbose: print("["+self._name+"] - "+'Managed system diagnose - "' + _description + '"')
            ...
            # 3b. PERFORM THE ADAPTATION ACTION ROUTINE
            #self.RaPLogger.log("["+self._name+"] - "+'Managed system adaptation - "' + _description + '"')
            if self._verbose: print("[" + self._name + "] - " + 'Managed system adaptation - "' + _description + '"')
            ...
            for p in _propertyList:
                try:
                    ...
                    if self._verbose: print("["+self._name+"] - "+'SUCCESS - ADAPTED property "' + p.name + '" from value "' + old.__str__() + '" to value "' + p.value.__str__()+ '"' )
                except:
                    #self.RaPLogger.log("["+self._name+"] - "+'ERROR - FAILED TO ADAPT property "' + p.name)
                    if self._verbose:print("["+self._name+"] - "+'ERROR - FAILED TO ADAPT property "' + p.name)
            ...
            if self._verbose: print("ERROR - Unknown action type")

        # 4. RETURN STATUS OF THE ACTION
        return _status
